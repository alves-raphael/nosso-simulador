...
const SacCalculator = (() => {
  function calcularCenario(valorFinanciado, taxaMensal, prazo, extrasMap, tipoAmortizacao) {
    ...
    let amortizacaoFixa = valorFinanciado / prazo;
    let totalJuros = 0;
    let totalPago  = 0;
    ...
    for (let mes = 1; mes <= prazo; mes++) {
      ...
      const amortRegular = Math.min(amortizacaoFixa, saldo);
      ...
      if (tipoAmortizacao === 'reduzir-parcela' && amortExtra > 0) {
        ...
        if (mesesRestantes > 0) {
          amortizacaoFixa = saldo / mesesRestantes;
        }
      }
    }
  ...
  function calcular({ valorFinanciado, taxaAnualPercent, prazo, amortizacoesExtras, tipoAmortizacao }) {
